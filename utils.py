    ...
    try:
        ...
        if version < required_version:
            raise AssertionError(
                f"diffusers version {version} is installed, but version {required_version} or higher is required."
            )
...
def apply_lora(
    ...
    for x in tqdm(to_load, desc="Loading model and applying LoRA weights:", leave=True):
        ...
        for param in params:
            name = name.replace(
                "._orig_mod.", "."
            )  # torch compiled modules have this prefix
            if low_mem_load:
                dtype_to_use = (
                    base_dtype
                ...
                try:
                    ...
                    continue
            model.patch_weight_to_device(
                "{}.{}".format(name, param), device_to=device_to
            )
            if low_mem_load:
                try:
                    set_module_tensor_to_device(
                        ...
                        device=transformer_load_device,
                        dtype=dtype_to_use,
                        value=model.model.diffusion_model.state_dict()[key],
                    )
                except:
                    continue
    ...
    if tiles > 1:
        ...
        for i in image_split:
            ...
            if not hasattr(embeds_split, "last_hidden_state"):
                ...
                embeds_split["last_hidden_state"] = torch.cat(
                    embeds_split["last_hidden_state"], encoded, dim=0
                )

        embeds_split["last_hidden_state"] = merge_hiddenstates(
            embeds_split["last_hidden_state"], tiles
        )
...
def get_sigmas(scheduler, steps, shift, riflex_freq_index=0):
    ...
    """
    # This logic is based on the schedulers found in the wanvideo codebase
    if "unipc" in scheduler:
        from .wanvideo.utils.fm_solvers import FlowUniPCMultistepScheduler

        sample_scheduler = FlowUniPCMultistepScheduler(shift=shift)
        sample_scheduler.set_timesteps(steps, shift=shift)
        sigmas = sample_scheduler.sigmas
    elif "euler" in scheduler:
        from .wanvideo.utils.fm_solvers import FlowMatchEulerDiscreteScheduler

        sample_scheduler = FlowMatchEulerDiscreteScheduler(shift=shift)
        sample_scheduler.set_timesteps(steps)
        sigmas = sample_scheduler.sigmas
    elif "dpm++" in scheduler:
        from .wanvideo.utils.fm_solvers import FlowDPMSolverMultistepScheduler

        sample_scheduler = FlowDPMSolverMultistepScheduler(shift=shift)
        sample_scheduler.set_timesteps(steps)
        sigmas = sample_scheduler.sigmas
    else:
    ...
    if riflex_freq_index > 0:
        ...
        time_filter = get_freq_filter(
            (steps, 1),
            riflex_freq_index,
            "inverse_linear",
            "cos",
        ).to(sigmas.device)
        sigmas = sigmas * time_filter.squeeze()
