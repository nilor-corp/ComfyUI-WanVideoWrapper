            ...
            if "blocks_" in main_part:
                ...
                if components[0] == "blocks":
                    ...
                    if idx < len(components):
                        if (
                            components[idx] == "self"
                            and idx + 1 < len(components)
                            and components[idx + 1] == "attn"
                        ):
                            new_key += ".self_attn"
                            idx += 2
                        elif (
                            components[idx] == "cross"
                            and idx + 1 < len(components)
                            and components[idx + 1] == "attn"
                        ):
                            new_key += ".cross_attn"
                ...
                if weight_type:
                    if weight_type == "alpha":
                        ...
                        weight_type == "lora_down.weight" or weight_type == "lora_down"
                    ):
...
class WanVideoLoraSelect:
    ...
    def getlorapath(
        self, lora, strength, blocks={}, prev_lora=None, low_mem_load=False
    ):
        loras_list = []
...
class WanVideoLoraBlockEdit:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": arg_dict,
            "optional": {
                "layer_filter": ("STRING", {"default": "", "multiline": True})
            },
        }
...
class WanVideoModelLoader:
    ...
    def loadmodel(
        ...
        assert not (
            vram_management_args is not None and block_swap_args is not None
        ...
        if (
            "vace_blocks.0.after_proj.weight" in sd
            and not "patch_embedding.weight" in sd
        ):
            raise ValueError(
                "You are attempting to load a VACE module as a WanVideo model, instead you should use the vace_model input and matching T2V base model"
            )

        if vace_model is not None:
            vace_sd = load_torch_file(
                vace_model["path"], device=transformer_load_device, safe_load=True
            )
            sd.update(vace_sd)
        ...
        if not "text_embedding.0.weight" in sd:
            ...
            "model_type.Wan2_1-FLF2V-14B-720P" in sd
            or "img_emb.emb_pos" in sd
            or "flf2v" in model.lower()
        ):
            model_type = "fl2v"
        ...
        if model_type == "i2v" or model_type == "fl2v":
            if (
                "480" in model
                or "fun" in model.lower()
                or "a2" in model.lower()
                or "540" in model
            ):  # just a guess for the Fun model for now...
        ...
        TRANSFORMER_CONFIG = {
            ...
            "in_dim_ref_conv": (
                sd["ref_conv.weight"].shape[1] if "ref_conv.weight" in sd else None
            ),
            "add_control_adapter": (
                True if "control_adapter.conv.weight" in sd else False
            ),
        }
        ...
        if "add_conv_in.weight" in sd:
            ...
            transformer.add_conv_in = torch.nn.Conv3d(
                add_cond_in_dim,
                inner_dim,
                kernel_size=transformer.patch_size,
                stride=transformer.patch_size,
            )
            transformer.add_proj = zero_module(torch.nn.Linear(inner_dim, inner_dim))
            transformer.attn_conv_in = torch.nn.Conv3d(
                attn_cond_in_dim,
                inner_dim,
                kernel_size=transformer.patch_size,
                stride=transformer.patch_size,
        ...
        if lora is not None:
            for l in lora:
                ...
                if l["blocks"]:
                    lora_sd = filter_state_dict_by_blocks(
                        lora_sd, l["blocks"], l.get("layer_filter", [])
                    )
                ...
                if "diffusion_model.patch_embedding.lora_A.weight" in lora_sd:
                    ...
                    new_in_dim = lora_sd[
                        "diffusion_model.patch_embedding.lora_A.weight"
                    ].shape[1]
                    ...
                    new_in.weight[:, :old_in_dim].copy_(
        ...
        if "fast" in quantization:
            ...
            convert_fp8_linear(
                patcher.model.diffusion_model, base_dtype, params_to_keep=params_to_keep
            )
        ...
        if compile_args is not None and vram_management_args is None:
            ...
            try:
                if hasattr(torch, "_dynamo") and hasattr(torch._dynamo, "config"):
                    torch._dynamo.config.recompile_limit = compile_args[
            ...
            if compile_args["compile_transformer_blocks_only"]:
                ...
                if vace_layers is not None:
                    for i, block in enumerate(
                        patcher.model.diffusion_model.vace_blocks
                    ):
                        patcher.model.diffusion_model.vace_blocks[i] = torch.compile(
                            block,
                            fullgraph=compile_args["fullgraph"],
                            dynamic=compile_args["dynamic"],
                            backend=compile_args["backend"],
        ...
        if (
            ...
            and patcher.model.diffusion_model.device != offload_device
        ):
            log.info(
                f"Moving diffusion model from {patcher.model.diffusion_model.device} to {offload_device}"
            )
                ...
                if key.startswith("encoder.block."):
                    ...
                    if "layer.0.SelfAttention" in key:
                        if key.endswith(".k.weight"):
                            ...
                            new_key = (
                                f"blocks.{block_num}.pos_embedding.embedding.weight"
                            )
                        else:
                            new_key = key
...
class WanVideoApplyNAG:
    ...
    def process(
        self, original_text_embeds, nag_text_embeds, nag_scale, nag_tau, nag_alpha
    ):
    ...
    def process(self, positive, negative=None):
        ...
        prompt_embeds_dict = {
            ...
            "negative_prompt_embeds": (
                negative[0][0].to(device) if negative is not None else None
            ),
        }
        return (prompt_embeds_dict,)
...
class WanVideoImageResizeToClosest:
        ...
        return {
            "required": {
                ...
                "aspect_ratio_preservation": (
                    ["keep_input", "stretch_to_new", "crop_to_new"],
                ),
            },
        }
    ...
    def process(
        self, image, generation_width, generation_height, aspect_ratio_preservation
    ):
        ...
        if aspect_ratio_preservation == "keep_input":
            ...
            or aspect_ratio_preservation == "crop_to_new"
        ):
        ...
        resized_image = common_upscale(
            image.movedim(-1, 1), w, h, "lanczos", crop
        ).movedim(1, -1)
...
class WanVideoClipVisionEncode:
    ...
    def process(
        ...
        if tiles > 0:
            ...
            clip_embeds = clip_encode_image_tiled(
                clip_vision, image.to(device), tiles=tiles, ratio=ratio
            )
            if negative_image is not None:
                negative_clip_embeds = clip_encode_image_tiled(
                    clip_vision, negative_image.to(device), tiles=tiles, ratio=ratio
                )
        else:
            if isinstance(clip_vision, ClipVisionModel):
                clip_embeds = clip_vision.encode_image(
                    image
                ).penultimate_hidden_states.to(device)
                if negative_image is not None:
                    negative_clip_embeds = clip_vision.encode_image(
                        negative_image
                    ).penultimate_hidden_states.to(device)
            else:
                pixel_values = clip_preprocess(
                    ...
                    mean=image_mean,
                    std=image_std,
                    crop=(not crop == "disabled"),
                ).float()
                clip_embeds = clip_vision.visual(pixel_values)
                if negative_image is not None:
                    pixel_values = clip_preprocess(
                        negative_image.to(device),
                        size=224,
                        mean=image_mean,
                        std=image_std,
                        crop=(not crop == "disabled"),
        ...
        if clip_embeds.shape[0] > 1:
            ...
            if clip_embeds.shape[0] > 2:
                for i in range(2, clip_embeds.shape[0]):
                    weighted_embeds.append(
                        clip_embeds[i : i + 1]
                    )  # Add as-is without strength modifier
...
class WanVideoImageToVideoEncode:
    ...
    def process(
        ...
        base_frames = num_frames + (
            1 if two_ref_images and not fun_or_fl2v_model else 0
        )
        if temporal_mask is None:
            ...
            mask = common_upscale(
                temporal_mask.unsqueeze(1).to(device),
                lat_w,
                lat_h,
                "nearest",
                "disabled",
            ...
            if mask.shape[0] > base_frames:
                ...
                mask = torch.cat(
                    [
                        mask,
                        torch.zeros(
                            base_frames - mask.shape[0], lat_h, lat_w, device=device
                        ),
                    ]
                )
        ...
        start_mask_repeated = torch.repeat_interleave(
            mask[:, 0:1], repeats=4, dim=1
        )  # T, C, H, W
        if end_image is not None and not fun_or_fl2v_model:
            end_mask_repeated = torch.repeat_interleave(
                mask[:, -1:], repeats=4, dim=1
            )  # T, C, H, W
            mask = torch.cat(
                [start_mask_repeated, mask[:, 1:-1], end_mask_repeated], dim=1
            )
        else:
        ...
        if start_image is not None:
            ...
            if noise_aug_strength > 0.0:
                resized_start_image = add_noise_to_reference_video(
        ...
        if end_image is not None:
            ...
            if noise_aug_strength > 0.0:
                resized_end_image = add_noise_to_reference_video(
        ...
        if temporal_mask is None:
            if start_image is not None and end_image is None:
                zero_frames = torch.zeros(
                    3, num_frames - start_image.shape[0], H, W, device=device
                )
                concatenated = torch.cat(
                    [resized_start_image.to(device), zero_frames], dim=1
                )
            elif start_image is None and end_image is not None:
                zero_frames = torch.zeros(
                    3, num_frames - end_image.shape[0], H, W, device=device
                )
                concatenated = torch.cat(
                    [zero_frames, resized_end_image.to(device)], dim=1
                )
                ...
                concatenated = torch.cat(
                    [
                        resized_start_image.to(device),
                        zero_frames,
                        resized_end_image.to(device),
                    ],
                    dim=1,
                )
        else:
            temporal_mask = common_upscale(
                temporal_mask.unsqueeze(1), W, H, "nearest", "disabled"
            ).squeeze(1)
            concatenated = resized_start_image[:, :num_frames] * temporal_mask[
                :num_frames
            ].unsqueeze(0)

        y = vae.encode(
            [concatenated.to(device=device, dtype=vae.dtype)],
            device,
        ...
        if extra_latents is not None:
            ...
            mask = torch.cat(
                [torch.ones_like(mask[:, 0 : samples.shape[1]]), mask], dim=1
            )
        ...
        if realisdance_latents is not None:
            realisdance_latents["ref_latent_neg"] = vae.encode(
        ...
        image_embeds = {
            "image_embeds": y,
            "clip_context": (
                clip_embeds.get("clip_embeds", None)
...
class WanVideoPhantomEmbeds:
    ...
    def process(
        ...
        phantom_end_percent,
        phantom_latent_1,
        phantom_latent_2=None,
        phantom_latent_3=None,
        phantom_latent_4=None,
        vace_embeds=None,
    ):
        vae_stride = (4, 8, 8)
        ...
        if phantom_latent_2 is not None:
            samples = torch.cat(
                [samples, phantom_latent_2["samples"].squeeze(0)], dim=1
            )
        if phantom_latent_3 is not None:
            samples = torch.cat(
                [samples, phantom_latent_3["samples"].squeeze(0)], dim=1
            )
        if phantom_latent_4 is not None:
            samples = torch.cat(
                [samples, phantom_latent_4["samples"].squeeze(0)], dim=1
            )
...
class WanVideoVACEEncode:
    ...
    def process(
        ...
        if input_frames is None:
            ...
            input_frames = (
                input_frames.to(self.vae.dtype)
                .to(self.device)
                .unsqueeze(0)
                .permute(0, 4, 1, 2, 3)
            )  # B, C, T, H, W
        ...
        if input_masks is None:
            ...
            input_masks = common_upscale(
                input_masks.clone().unsqueeze(1),
                width,
                height,
                "nearest-exact",
                "disabled",
        ...
        if ref_images is not None:
            ...
            if ref_images.shape[0] > 1:
                ref_images = torch.cat(
                    [ref_images[i] for i in range(ref_images.shape[0])], dim=1
                ).unsqueeze(0)
            ...
            if current_aspect > target_aspect:
                ...
                ref_images = padded
            ref_images = common_upscale(
                ref_images.movedim(-1, 1), width, height, "lanczos", "center"
            ).movedim(1, -1)

            ref_images = (
                ref_images.to(self.vae.dtype)
                .to(self.device)
                .unsqueeze(0)
                .permute(0, 4, 1, 2, 3)
                .unsqueeze(0)
        ...
        z0 = self.vace_encode_frames(
            input_frames, ref_images, masks=input_masks, tiled_vae=tiled_vae
        )
        ...
        vace_input = {
            ...
            "vace_seq_len": math.ceil(
                (z[0].shape[2] * z[0].shape[3]) / 4 * z[0].shape[1]
            ),
        ...
        if prev_vace_embeds is not None:
            if (
                "additional_vace_inputs" in prev_vace_embeds
                and prev_vace_embeds["additional_vace_inputs"]
            ):
                vace_input["additional_vace_inputs"] = prev_vace_embeds[
                    "additional_vace_inputs"
                ].copy()
    ...
    def vace_encode_frames(self, frames, ref_images, masks=None, tiled_vae=False):
        ...
        for latent, refs in zip(latents, ref_images):
            if refs is not None:
                if masks is None:
                    ref_latent = self.vae.encode(
                        refs, device=self.device, tiled=tiled_vae
                    )
                else:
...
class WanVideoVACEStartToEndFrame:
    ...
    def process(
        ...
        if control_images is not None:
            control_images = common_upscale(
                control_images.movedim(-1, 1), W, H, "lanczos", "disabled"
            ).movedim(1, -1)
        ...
        if start_image is not None and end_image is not None:
            ...
            if control_images is None:
                ...
                empty_frames = control_images[
                    start_image.shape[0] : num_frames - end_image.shape[0]
                ]
            ...
            if control_images is None:
                empty_frames = (
                    torch.ones(
                        (num_frames - start_image.shape[0], H, W, 3), device=device
                    )
                    * empty_frame_level
                )
            else:
            ...
            if control_images is None:
                empty_frames = (
                    torch.ones(
                        (num_frames - end_image.shape[0], H, W, 3), device=device
                    )
                    * empty_frame_level
                )
            else:
        ...
        if inpaint_mask is not None:
            ...
            if inpaint_mask.shape[0] > num_frames:
                ...
                inpaint_mask = inpaint_mask.repeat(
                    num_frames // inpaint_mask.shape[0] + 1, 1, 1
                )[:num_frames]
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "verbose": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "Print debug output"},
                ),
            },
            "optional": {
                "vae": ("WANVAE",),
...
class CreateCFGScheduleFloatList:
    ...
    def process(
        ...
        for i in range(start_idx, end_idx + 1):
            ...
            cfg_list[i] = round(
                cfg_scale_start + factor * (cfg_scale_end - cfg_scale_start), 2
            )
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "drift_flow_shift": (
                    "FLOAT",
                    {"default": 3.0, "min": 1.0, "max": 30.0, "step": 0.01},
                ),
                "source_cfg": (
                    "FLOAT",
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "shift_skip": (
                    "INT",
                    {"default": 6, "min": 0, "tooltip": "Skip step of latent shift"},
                ),
                "start_percent": (
                    "FLOAT",
                    {
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "use_fresca": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "https://github.com/WikiChao/FreSca"},
                ),
                "fresca_scale_low": (
                    "FLOAT",
                ...
                "fresca_scale_high": (
                    "FLOAT",
                    {"default": 1.25, "min": 0.0, "max": 10.0, "step": 0.01},
                ),
                "fresca_freq_cutoff": (
                    "INT",
                    {"default": 20, "min": 0, "max": 10000, "step": 1},
                ),
            },
        }
...
class WanVideoDecode:
    ...
    def decode(
        ...
        if type(vae).__name__ == "TAEHV":
            images = vae.decode_video(latents.permute(0, 2, 1, 3, 4))[0].permute(
                1, 0, 2, 3
            )
        ...
        if is_looped:
            ...
            temp_images = (temp_images - temp_images.min()) / (
                temp_images.max() - temp_images.min()
            )
            out = temp_images[:, 9:]
...
class WanVideoEncode:
    ...
    def encode(
        ...
        if W % 16 != 0 or H % 16 != 0:
            ...
            new_width = (W // 16) * 16
            log.warning(
                f"Image size {W}x{H} is not divisible by 16, resizing to {new_width}x{new_height}"
            )
            image = common_upscale(
                image.movedim(-1, 1), new_width, new_height, "lanczos", "disabled"
            ).movedim(1, -1)

        image = (
            image.to(vae.dtype).to(device).unsqueeze(0).permute(0, 4, 1, 2, 3)
        )  # B, C, T, H, W
        if noise_aug_strength > 0.0:
        ...
        if isinstance(vae, TAEHV):
            latents = vae.encode_video(
                image.permute(0, 2, 1, 3, 4), parallel=False
            )  # B, T, C, H, W
        ...
        if mask is None:
            ...
            mask = torch.nn.functional.interpolate(
                mask.unsqueeze(0).unsqueeze(
                    0
                ),  # Add batch and channel dims [1,1,T,H,W]
                size=(latents.shape[2], target_h, target_w),
                mode="trilinear",
                align_corners=False,
            ).squeeze(
                0
            )  # Remove batch dim, keep channel dim
...
NODE_CLASS_MAPPINGS = {
    ...
    "TiledWanVideoSampler": TiledWanVideoSampler,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "TiledWanVideoSampler": "Tiled WanVideo Sampler",
